...
def build(MiroSystem, SPEEDMODE = False):
    H = 3.32
    wall_t = 0.1 * 2
    topWall_pos = np.array([0.6, 5/2*H, 5.1])
    if(SPEEDMODE):        
        ...
        pos_down = np.array([topWall_pos[0], topWall_pos[1] - 1.26, topWall_pos[2]])
        pos_up = np.array([topWall_pos[0], topWall_pos[1] + 1.16, topWall_pos[2]])
